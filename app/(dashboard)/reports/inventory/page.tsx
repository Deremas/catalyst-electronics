...
import { Package, ArrowLeft, Download, Filter, Search, Warehouse, ShieldAlert } from "lucide-react";
...
export default function InventoryReportsPage() {
  ...
  const totalItems = Array.from(new Set(filteredItems.map((item: any) => item.id))).length;
  const totalValuation = filteredItems.reduce((sum: number, item: any) => sum + (Number(item.stock || 0) * Number(item.price || 0)), 0);
  const lowStockCount = filteredItems.filter((item: any) => Number(item.stock || 0) <= Number(item.lowStockAlert || 10)).length;
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-slate-500 mt-1">Stock levels, valuation, and movement analysis.</p>
          </div>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <ReportSummary title="Total Items" value={totalItems.toLocaleString()} sub="Unique items filtered" icon={Package} />
        <ReportSummary title="Total Valuation" value={formatCurrency(totalValuation)} sub="Based on retail price" icon={Warehouse} color="emerald" />
        <ReportSummary title="Low Stock Items" value={lowStockCount.toLocaleString()} sub="Items below safety levels" icon={ShieldAlert} color="amber" />
      </div>
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-zinc-800">
              {filteredItems.map((item: any, idx: number) => {
                const location = locations.find((l: any) => l.id === item.locationId);
                const itemValuation = Number(item.stock || 0) * Number(item.price || 0);
                const isLowStock = Number(item.stock || 0) <= Number(item.lowStockAlert || 10);
                return (
                  <tr key={`${item.id}-${idx}`} className="transition-colors hover:bg-slate-50/50 dark:hover:bg-zinc-800/30">
                    <td className="px-6 py-4 text-xs font-bold text-slate-900 dark:text-white">{item.name}</td>
                    ...
                    <td className="px-6 py-4 text-right text-xs font-bold text-slate-900 dark:text-zinc-100">{formatCurrency(Number(item.price || 0))}</td>
                    <td className={`px-6 py-4 text-center text-xs font-black ${isLowStock ? 'text-rose-500' : 'text-slate-700 dark:text-zinc-300'}`}>
                      {item.stock} {item.unit}
                    </td>
                    <td className="px-6 py-4 text-right text-xs font-bold text-slate-900 dark:text-zinc-100">{formatCurrency(itemValuation)}</td>
                  </tr>
                );
              })}
